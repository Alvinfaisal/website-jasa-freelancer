...
class AddForeignKeysToOrder extends Migration
{
  ...
  public function up()
  {
    Schema::table('order', function (Blueprint $table) {
      //script foreign keys order table to users, service and order status table
      $table->foreign('buyer_id', 'fk_order_buyer_to_users')->references('id')->on('users')->onUpdate('CASCADE')->onDelete('CASCADE');

      $table->foreign('frelancer_id', 'fk_order_frelancer_to_users')->references('id')->on('users')->onUpdate('CASCADE')->onDelete('CASCADE');
  ...
  public function down()
  {
    Schema::table('order', function (Blueprint $table) {
      ...
      $table->dropForeign('fk_order_frelancer_to_users');
